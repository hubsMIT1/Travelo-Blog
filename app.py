...
#Each post page
@app.route('/posts/<string:post_title>',methods=['GET','POST'])
    ...
    if 'loggedin' in session:
        if 'post_id' in request.args:
            ...
            if request.method == 'POST':
                ...
                comment_date = temp_date.strftime('%d %B %Y at %H:%M')
                # there is to create/fetch comment from db
                cursor.execute('INSERT INTO comment (comment_user,comment, post_id, comment_date)' 
                           'VALUES (%s, %s, %s, %s)',
                            (user, comment, post_id, comment_date))
                conn.commit()
            cursor.execute('SELECT * FROM comment WHERE post_id = %s', [post_id])
            comments = cursor.fetchall()
            return render_template('postItem.html', postItem = postItem, comments = comments)        
...
def edit(id):
    ...
    cursor.execute('SELECT username FROM post where post_id =%s', [id])
    ...
    if request.method != 'POST':
        ...
        edit_post = cursor.fetchone()
        return render_template('edit_post.html', post = edit_post)

    editTitle = request.form['title-post']
    editCaption = request.form['caption']
    
    cursor.execute('UPDATE post SET post_title = %s, caption = %s WHERE post_id = %s', (editTitle, editCaption, id))
    conn.commit()
    return redirect(url_for('postList'))
...
def delete_comment(id):
    ...
    cursor.execute('SELECT comment_user FROM comment where comment_id =%s', [id])
    username = cursor.fetchone()
    ...
    return redirect(url_for('postItem',post_title = post_title,post_id = post_id))
